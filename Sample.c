...
// Reads money like: 1000, 1000.5, 1000.50  => cents
static int read_money_cents(const char *prompt, cents_t *centsOut) {
    char buf[64];
    read_line(prompt, buf, sizeof(buf));
    if (buf[0] == '\0') return 0;

    // allow optional leading +/-, digits, optional . and up to 2 digits
    int sign = 1;
    char *p = buf;
    ...
    if (*p == '.') {
        ...
        // if there are more than 2 fractional digits -> invalid
        if (isdigit((unsigned char)*p)) return 0;
    }
    ...
    else if (fracDigits == 1) frac *= 10; // e.g. ".5" => 50 cents
...
static void format_money(cents_t cents, char *out, size_t outSize) {
    // prints like 1234.56 (supports negative)
...
/*
Safe update pattern for text files:
  - read accounts.txt
  - write to temp file with modifications
  - replace original file
*/
static int rewrite_accounts_with_update(
    int targetAccount,
    int (*mutator)(BankAccount *acc, void *ctx), // return 1 if modified
    void *ctx,
    int requireFound
) {
    ...
    if (!in) {
        ...
        // If file doesn't exist and not required, create empty base
    ...
    while (fgets(line, sizeof(line), in)) {
        if (!parse_account_line(line, &acc)) {
            // skip bad lines (or you could copy them)
            continue;
        }
    ...
    if (remove(ACCOUNTS_FILE) != 0) {
        // If remove fails, try overwrite anyway
    }
...
static void deposit(void) {
    ...
    MoneyCtx ctx = { amount };
    int ok = rewrite_accounts_with_update(accountNumber, deposit_mutator, &ctx, /*requireFound=*/1);
...
static void withdraw_money(void) {
    ...
    int found = rewrite_accounts_with_update(accountNumber, withdraw_mutator, &ctx, /*requireFound=*/1);
...
static void update_holder_name(void) {
    ...
    int ok = rewrite_accounts_with_update(accountNumber, rename_mutator, &ctx, /*requireFound=*/1);
    if (!ok) {
        printf("Account not found.\n");
...
static void delete_account(void) {
    ...
    // Rewrite file excluding this account
    FILE *in = fopen(ACCOUNTS_FILE, "r");
    if (!in) {
...
static void transfer_money(void) {
    ...
    // Step 2: deposit into TO
    MoneyCtx dctx = { amount };
    int ok2 = rewrite_accounts_with_update(toAcc, deposit_mutator, &dctx, 1);
    if (!ok2) {
        // Rare case: TO missing after we checked. Rollback is complex; at least log loudly.
...
static void view_transactions(void) {
    ...
    printf("\n--- Transaction History (latest first not guaranteed) ---\n");
    ...
    int count = 0;
    while (fgets(line, sizeof(line), f)) {
        // Print as-is (already structured)
        printf("%s", line);
        count++;
        if (count >= 200) { // prevent runaway output
